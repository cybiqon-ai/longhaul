...
function DayStrip({ project }: { project: ProjectRow }) {
  ...
  return (
    <div className="mt-3 flex h-1.5 overflow-hidden rounded-full bg-[--color-line-2]">
      {order.map((status) =>
        counts[status] ? (
          <span
            key={status}
  ...
  const body = (
    <Card
      className={cx(
        ...
        !broken && "hover:border-[--color-accent]",
        broken && "opacity-70"
      )}
    >
      <div className="flex items-start gap-2">
        <div className="min-w-0">
          ...
          <p className="truncate font-mono text-xs text-[--color-muted]">{project.path}</p>
        </div>
        {needsYou > 0 && (
          <span className="ml-auto flex shrink-0 items-center gap-1 rounded-full border border-[--color-parked]/50 px-2 py-0.5 text-xs text-[--color-parked]">
      ...
      {broken ? (
        ...
        <>
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-[--color-ink-2]">
